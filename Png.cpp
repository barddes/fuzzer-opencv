...
private
    ...
    uint Crc32(byte[] stream, int offset, int length, uint crc) {
        uint c;
        if (crcTable == null) {
...
public
    ...
    void SetFilter(int y, byte filter) {
...
public
    ...
    void SetPixel(int x, int y, byte[] pixel) {
        ...
        if (pixel.Length >= 4) {
            ...
            SetPixel(x, y, pixel[0], pixel[1], pixel[2], (byte)
            255);
        } else {
